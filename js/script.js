...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupInfiniteCarousel(
    ...
    // We clone enough cards to fill the view on both sides
    const numToClone = Math.max(3, originalCards.length);

    // Append clones of the first few cards to the end
    ...
    // Prepend clones of the last few cards to the start
    for (let i = 0; i < numToClone; i++) {
    ...
    const getCardWidth = () => {
      ...
      return firstCard ? firstCard.offsetWidth + 32 : 300 + 32; // 32 is the gap
    };

    // Position the scroll at the first "real" card
    const initialPosition = numToClone * getCardWidth();
    ...
    const handleInfiniteScroll = () => {
      ...
      // If we scrolled into the start clones (left side)
      if (scrollLeft <= (numToClone - 1) * cardWidth) {
        container.style.scrollBehavior = "auto";
        container.scrollLeft = scrollLeft + totalOriginalWidth;
        container.style.scrollBehavior = "smooth";
      }
      // If we scrolled into the end clones (right side)
      else if (scrollLeft >= (numToClone + originalCards.length) * cardWidth) {
        container.style.scrollBehavior = "auto";
        container.scrollLeft = scrollLeft - totalOriginalWidth;
        container.style.scrollBehavior = "smooth";
      }
    ...
    // Check for jumps after smooth scrolls or manual swipes
    let scrollTimeout;
    ...
    // Handle window resize to keep scroll position stable
    window.addEventListener("resize", () => {
      const cardWidth = getCardWidth();
      container.style.scrollBehavior = "auto";
      // We might need to recalibrate here, but simple reset to start of real cards is safest
      // but for now let's just let it be.
    });
  }

  // Initialize all carousels as perfect infinite loops
  setupInfiniteCarousel(
    ".portfolio-cards",
    ".portfolio-card",
    ".portfolio-carousel .portfolio-carousel-prev, .portfolio-carousel .carousel-btn-prev",
    ".portfolio-carousel .portfolio-carousel-next, .portfolio-carousel .carousel-btn-next"
  );

  setupInfiniteCarousel(
    ".blogs-cards",
    ".blog-card",
    ".blogs-carousel .blogs-carousel-prev, .blogs-carousel .carousel-btn-prev",
    ".blogs-carousel .blogs-carousel-next, .blogs-carousel .carousel-btn-next"
  );

  setupInfiniteCarousel(
    ".testimonials-cards",
    ".testimonial-card",
    ".testimonials-carousel .testimonials-carousel-prev, .testimonials-carousel .carousel-btn-prev",
    ".testimonials-carousel .testimonials-carousel-next, .testimonials-carousel .carousel-btn-next"
  );
  ...
  if (newsletterBtn) {
    newsletterBtn.addEventListener("click", function (e) {
      ...
      if (email && email.includes("@")) {
        ...
        alert(
          "Thank you for subscribing! We will keep you updated with the latest energy insights."
        );
  ...
  function initParallax() {
    ...
    const heroImages = document.querySelectorAll(
      ".hero-bg-img, .about-hero-img, .projects-hero-img"
    );

    // Handle hanging images separately to preserve centering
    const hangingImageContainers = document.querySelectorAll(
      ".hero-hanging-image"
    );
          ...
          window.requestAnimationFrame(function () {
            ...
            heroImages.forEach((img) => {
              ...
              const container = img.closest(
                ".hero-section, .projects-hero-section, .hero-bg"
              );
  ...
  const animateElements = document.querySelectorAll(
    ".service-card, .portfolio-card, .testimonial-card, .why-choose-card, .leadership-card, .service-card-white, .content-image-wrapper, .newsletter-cta-box"
  );
  ...
  function initStaggeredAnimations() {
    ...
    const cardContainers = document.querySelectorAll(
      ".services-grid, .portfolio-cards, .blogs-cards, .testimonials-cards, .why-choose-grid, .leadership-grid, .timeline-content, .services-grid-3"
    );

    cardContainers.forEach((container) => {
      // Skip containers that are projects-grid or blog-grid (handled by two-column animations)
      if (
        container.classList.contains("projects-grid") ||
        container.classList.contains("blog-grid")
      ) {
        return;
      }
      ...
      const cards = container.querySelectorAll(
        ".service-card, .portfolio-card, .blog-card, .testimonial-card, .why-choose-card, .leadership-card, .service-card-white"
      );
  ...
  function initTextAnimations() {
    ...
    const textElements = document.querySelectorAll(
      ".hero-title, .section-title, .portfolio-title, .blogs-title, .testimonials-title, .timeline-main-title, .leadership-header h2, .vision-content h2"
    );
  ...
  function initTimelineLineAnimation() {
    ...
    function updateTimeline() {
      ...
      const animationEnd =
        sectionTop + sectionHeight - windowHeight + startOffset;
  ...
  function initVisionSectionAnimation() {
    ...
    visionImageWrapper.style.transition =
      "opacity 0.8s cubic-bezier(0.4, 0, 0.2, 1), transform 0.8s cubic-bezier(0.4, 0, 0.2, 1)";
    ...
    visionContent.style.transition =
      "opacity 0.8s cubic-bezier(0.4, 0, 0.2, 1), transform 0.8s cubic-bezier(0.4, 0, 0.2, 1)";
